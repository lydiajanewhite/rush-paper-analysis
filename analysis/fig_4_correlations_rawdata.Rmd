...
# Correlations of functional vs compositional stability
...
So use all 3 caged treatments - which are the data used to calculate contributions. 

the focus of the paper is consumers so we look at comsumer treatments, pooling across stressors (which showed weaker effects than consumers)
...
gg3 <- ggscatter(filter(combined, metric == "Resist"), x="Composition", y="Function", 
   add = "reg.line",
   color = "Diversity", 
   cor.coef = F, 
   cor.method = "spearman",
...
dens3a <- ggplot((filter(combined, metric == "Resist")), aes(x = Composition, fill = Diversity)) + 
  geom_density(size=0.4, alpha = 0.7) +  
  theme_void() + 
  theme(legend.position = "none")

dens3b <- ggplot((filter(combined, metric == "Resist")), aes(x = Function, fill = Diversity)) + 
  geom_density(size=0.4, alpha = 0.7) +  
  theme_void() + 
  theme(legend.position = "none") +
  coord_flip()

gg4 <- ggscatter(filter(combined, metric == "Resilience"), x="Composition", y="Function", 
   add = "reg.line",
   color = "Diversity", 
   cor.coef = F, 
   cor.method = "spearman",
...
dens4a <- ggplot((filter(combined, metric == "Resilience")), aes(x = Composition, fill = Diversity)) + 
  geom_density(size=0.4, alpha = 0.7) +  
  theme_void() + 
  theme(legend.position = "none")

dens4b <- ggplot((filter(combined, metric == "Resilience")), aes(x = Function, fill = Diversity)) + 
  geom_density(size=0.4, alpha = 0.7) +  
  theme_void() + 
  theme(legend.position = "none") +
  coord_flip()
...
corr_plot <- dens1a + plot_spacer() + dens2a + plot_spacer() +
  ...
  dens3a + plot_spacer() + dens4a + plot_spacer() +
  gg3 + dens3b + gg4 + dens4b +
  plot_layout(ncol = 4, nrow = 4, widths = c(4, 1, 4, 1), heights = c(1, 4))
```
 
 Can also look at relationships between metrics.....
 But not gonna include this in the paper.
...
combinations <- combn(c("Temp.Var", "Resist", "Resilience"), 2) %>%   # t = transpose
  t() %>% 
  data.frame() %>% 
  crossing(c("Function", "Composition"), .) %>% 
  rename_with(~ c("response","var_x","var_y"))

corr_and_shapiro <- function( x, y, type, data_tab) {
  data_tmp <- filter(data_tab, response == type)
  x <- pull(data_tmp, x)
  y <- pull(data_tmp, y)
  tmp<-cor.test(x, y, method = "spearman")
  norm.x<-shapiro.test(x)
  norm.y<-shapiro.test(y)
  
  data.frame(
    normality_x=norm.x$p.value,
    normality_y=norm.y$p.value,
    Pval = tmp$p.value,
    Rho = tmp$estimate,
    test = tmp$method
    )
...
combinations %>% 
  rowwise() %>% 
  mutate(corr_and_shapiro(var_x, var_y, response, test))
```
...
ggpairs(filter(test, response =="Composition"), mapping = aes(color = Diversity), columns = 6:8, method = "spearman")
ggpairs(filter(test, response =="Function"), mapping = aes(color = Diversity), columns = 6:8, method = "spearman")
