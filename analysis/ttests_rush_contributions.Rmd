...
t-tests rely on exactly the same confidence intervals that we have plotted so, grazer (i.e Diversity treat A) contributions to temp var (no stressor and nutrients) are less than zero. Grazer contribution to spa var (no stressor) and predator contribution to spa var (sediment) are greater than zero.  
```{r}
test<-contributions %>% 
...
p.adjust(test$pval [1:4],method="BH")
p.adjust(test$pval [5:8],method="BH") # 
p.adjust(test$pval [9:14],method="BH") # Spa.Var A_N0_S0 no longer sig
p.adjust(test$pval [15:20],method="BH") # Temp.Var A_N1_S0 no longer sig 
...
p.adjust(test$pval [1:4],method="BH") # Resilience grazers no longer greater than zero 
p.adjust(test$pval [5:8],method="BH")  
p.adjust(test$pval [9:14],method="BH") 
p.adjust(test$pval [15:20],method="BH") 
