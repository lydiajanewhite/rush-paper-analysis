...
t-tests rely on the same confidence intervals that we have plotted so, grazer (i.e Diversity treat A) contributions to temp var (no stressor and nutrients) are less than zero. Grazer contribution to spa var (no stressor) and predator contribution to spa var (sediment) are greater than zero.  
but with multiple comparisons p values are adjusted and some no longer significant. 
```{r}
...
test_func %>%  
  ...
  mutate(pval.adj = p.adjust (pval, method='BH'))  # reduce comparisons from 6/4 to 3/2 ie each stressor treat within diversity 
 
comparisons_func<-test_func %>%  
  group_by(metric) %>% 
  mutate(pval.adj = p.adjust (pval, method='BH'))%>% 
  group_by(metric, Diversity) %>% 
  mutate(pval.adj_A = p.adjust (pval, method='BH'))%>% 
  ...
  group_by(metric, Stressor) %>% 
  mutate(pval.adj_B = p.adjust (pval, method='BH')) %>% 
...
contributions %>% 
  ...
  summarise(pval = t.test(x = LRR_metric, mu = 0)$p.value, 
            tval = t.test(x = LRR_metric, mu = 0)$statistic,
            conf = t.test(x = LRR_metric, mu = 0)$conf.int)

test_func <- test_func %>% 
  separate(Treatment, c("Diversity", "Nutrients", "Sediment"), sep='_') %>% 
  arrange(desc(metric), Nutrients, Sediment, desc(Diversity))

print.data.frame(test_func)
test_func$tval

test_func$response <- "Functional" 


#write.csv(test, file = "ttest_data_function.csv")

test_func_edit<-contributions %>% 
  filter(Treatment != "A_N1_S1" & Treatment != "B_N1_S1")%>% 
  separate(Treatment, c("Diversity", "Nutrients", "Sediment"), sep='_', remove = FALSE) %>% 
  unite(Stressor, c("Nutrients", "Sediment"), sep = "_")%>% 
  group_by(metric, Diversity, Stressor, Treatment) %>% 
  summarise(pval = t.test(x = LRR_metric, mu = 0)$p.value,  # default is two.sided 
            tval = t.test(x = LRR_metric, mu = 0)$statistic,
            norm = shapiro.test(x = LRR_metric)$p.value)
...
Can do t-test for predicted and observed, again to see what is diff from zero. 
But doesn't work cos of samples sizes for bootstrapped data. 

Then for predator contribution to spa var. will check for equal variance first (is equal) so continue with standard 2 sample t test. 
After accounting for 2 comparisons with BH. 
For all other metrics predict vs observed are not sig different. 

t_test is just a pipeable wrapper for t.test (package = rstatix) but cannot manually dictate how many comparisons to adjust p vals for. 
...
var.test(filter(predict, metric == "Spa.Var" & Diversity == "B", type == "N1_S1")$LRR_metric, filter(predict, metric == "Spa.Var" & Diversity == "B", type == "Predicted")$LRR_metric)
...
# predict %>% 
#  filter(metric == "Resilience" & Diversity == "A") %>% 
#  t_test(LRR_metric ~ type )
...
t-tests rely on exactly the same confidence intervals that we have plotted so, grazer (i.e Diversity treat A) contributions to resilience (both sediment and nutrients) are greater than zero, but predator contribution to resilience (sediment) is only marginal p = 0.06. All consumer contributions to spatial var are different from zero apart from predator/sediment (p = 0.069). 

For spatial variability grazer vs predator contributions are different (for sediment) from one another despite predators not being different from zero. 
...
test_comp$response <- "Compositional" 
  
print.data.frame(test_comp)

ttest_data<-rbind(test_func, test_comp)

write.csv(ttest_data, file = "ttest_data.csv")

test_comp_edit<-contributions_Comp %>% 
  filter(Treatment != "A_N1_S1" & Treatment != "B_N1_S1")%>% 
  separate(Treatment, c("Diversity", "Nutrients", "Sediment"), sep='_', remove = FALSE) %>% 
  unite(Stressor, c("Nutrients", "Sediment"), sep = "_")%>% 
  group_by(metric, Diversity, Stressor, Treatment) %>% 
...
var.test(filter(contributions_Comp, metric == "Spa.Var" & Treatment == "B_N0_S1")$LRR_metric, 
         filter(contributions_Comp, metric == "Spa.Var" & Treatment == "A_N0_S1")$LRR_metric)

test1<-contributions_Comp %>% 
  filter(metric == "Spa.Var" & Treatment == "B_N0_S1" |metric == "Spa.Var" & Treatment == "A_N0_S1" )%>% 
  t_test(LRR_metric ~ Treatment)

p.adjust(test1$p, n = 3)  # 3 comparisons between grazers and predators, one for each stressor treatment. 
...
test2<-contributions_Comp %>% 
  filter(metric == "Spa.Var" & Treatment == "A_N0_S1" |metric == "Spa.Var" & Treatment == "A_N1_S0" )%>% 
  t_test(LRR_metric ~ Treatment)

p.adjust(test2$p, n = 3)  # 3 comparisons between grazers stressor treatments. 
...
test3<-contributions_Comp %>% 
  filter(metric == "Spa.Var" & Treatment == "A_N1_S0" |metric == "Spa.Var" & Treatment == "A_N0_S0" )%>% 
  t_test(LRR_metric ~ Treatment)

p.adjust(test3$p, n = 3)

var.test(filter(contributions_Comp, metric == "Spa.Var" & Treatment == "A_N0_S1")$LRR_metric, 
         filter(contributions_Comp, metric == "Spa.Var" & Treatment == "A_N0_S0")$LRR_metric)

test4<-contributions_Comp %>% 
  filter(metric == "Spa.Var" & Treatment == "A_N0_S1" |metric == "Spa.Var" & Treatment == "A_N0_S0" )%>% 
  t_test(LRR_metric ~ Treatment)

p.adjust(test4$p, n = 3)


check <-contributions_Comp %>% 
  filter(Treatment != "A_N1_S1" & Treatment != "B_N1_S1") %>% 
   filter(metric == "Spa.Var") %>% 
    t_test (LRR_metric ~ Treatment, p.adj = "BH")
...
can do 2 tailed t-test for predicted vs observed, for grazer contribution to spa var. will check for equal variance first (is equal) so continue with standard 2 sample t test. 
also check that both are different from zero... 
```{r}

var.test(filter(predict_Comp, metric == "Spa.Var" & Diversity == "B", type == "N1_S1")$LRR_metric, filter(predict, metric == "Spa.Var" & Diversity == "B", type == "Predicted")$LRR_metric)
...
#  filter(metric == "Spa.Var" & Diversity == "A")%>% 
#  group_by(type) %>% 
#  summarise (t_test(LRR_metric ~ 1, mu = 0))
