...
comparisons_func<-test_func %>%  
  ...
  group_by(metric, Stressor) %>%    # 3/2 comparisons n of diversity treats
...
#  group_by(metric) %>%      
#  mutate(pval.adj = p.adjust (pval, method='BH'))%>%   # 6/4 comparisons n of treats
#  group_by(metric, Diversity) %>%     
#  mutate(pval.adj_A = p.adjust (pval, method='BH'))%>%  # 3/2 comparisons n of stressors 
   
fd<-contributions %>% 
  filter(metric == "Spa.Var" & Treatment == "A_N0_S0" )%>% 
  t_test(LRR_metric ~ 1, mu = 0) 

fd$p
p.adjust(fd$p, 'BH', n = 2) # check that is adjusting for correct no of groups i.e. 2. 
